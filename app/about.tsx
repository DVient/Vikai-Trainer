...
const FEATURES: Section = {
  ...
  items: [
    ...
    "🎨 Team skin — paint the whole app in your two team colors (⚙️ on Home). Text always picks the color that's easiest to read.",
    ...
    <View className="rounded-2xl border border-edge bg-card p-4">
      ...
      <View className="mt-2 gap-2">
        {section.items.map((item) => (
          <Text key={item} className="text-sm leading-5 text-body">
            {item}
          </Text>
        ))}
      </View>
...
export default function About() {
  return (
    <ScrollView className="flex-1 bg-app" contentContainerClassName="items-center px-4 pb-8">
      <View className="w-full max-w-md gap-4">
        <View className="rounded-2xl border-2 border-go-line bg-go-soft p-4">
          <Text className="text-lg font-black text-go">{HERO.title}</Text>
          <Text className="mt-1 text-sm leading-5 text-body">{HERO.body}</Text>
        </View>
